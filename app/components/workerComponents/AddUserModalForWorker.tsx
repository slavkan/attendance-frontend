...
interface AddUserModalForWorkerProps {
...
export default function AddUserModalForWorker({
  token,
  opened,
  open,
  close,
  ...
  const [newUserToFaculty, setNewUserToFaculty] = useState(false);
  ...
  const onSubmit = async (e: React.ChangeEvent<any>) => {
    ...
    try {
      ...
      if (response.ok) {
        setNewUserToFaculty(true);
        const newUser = await response.json();
        addUserToFaculty(newUser.id, facultyId);
        notifications.show({
          withBorder: true,
  ...
  const addUserToFaculty = async (userId: string, facultyId: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/faculty-person?personId=${userId}&facultyId=${facultyId}`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
      ...
      if (response.ok) {
        ...
        notifications.show({
          ...
          message: `Korisnik ${newPersonForm.firstName} ${newPersonForm.lastName} je uspješno dodan na fakultet`,
        });
      } else {
        const errorData = await response.json();
        if (errorData) {
